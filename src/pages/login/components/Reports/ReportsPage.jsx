...
const localAvatars = import.meta.glob('../../assets/*.{png,jpg,jpeg,svg}', { eager: true, import: 'default' });
const avatarMap = Object.fromEntries(
  Object.entries(localAvatars).map(([path, url]) => {
    const file = path.split('/').pop();
    const base = file ? file.split('.').slice(0, -1).join('.').toLowerCase() : '';
    return [base, url];
  })
);
...
const teamsMeta = [
  { id: 1, key: "coding", name: "Coding Team", department: "Software Development", color: "bg-blue-500", icon: Code },
  { id: 2, key: "proposal", name: "Proposal Team", department: "Research Proposals", color: "bg-green-500", icon: FileText },
  { id: 3, key: "journal", name: "Journal Team", department: "Research Publications", color: "bg-purple-500", icon: BookOpen },
  { id: 4, key: "writing", name: "Writing Team", department: "Technical Documentation", color: "bg-yellow-500", icon: Edit2 },
];
...
const ReportsPage = () => {
  ...
  const [reportData, setReportData] = useState({
    period: "",
    teams: {
      coding: { total: 0, active: 0, completed: 0, createdInPeriod: 0, finishedInPeriod: 0, itemsInPeriod: [] },
      proposal: { total: 0, active: 0, completed: 0, createdInPeriod: 0, finishedInPeriod: 0, itemsInPeriod: [] },
      journal: { total: 0, active: 0, completed: 0, createdInPeriod: 0, finishedInPeriod: 0, itemsInPeriod: [] },
      writing: { total: 0, active: 0, completed: 0, createdInPeriod: 0, finishedInPeriod: 0, itemsInPeriod: [] }
    },
  ...
  const getReportTitle = () => {
    if (reportData?.period) {
      const label = showDateRange ? "Custom" : 
                   timeFilter === "weekly" ? "Weekly" : 
                   timeFilter === "monthly" ? "Monthly" : "Yearly";
      return `${label} Report (${reportData.period})`;
    }
    return "Performance Reports";
  };

  // In the useEffect where you load reports, add better logging
useEffect(() => {
  let isMounted = true;
  ...
  const loadReports = async () => {
    try {
      ...
      let data;
      // All functions now return the same data (no date filtering)
      ...
      if (isMounted && data) {
        ...
        if (data.teams?.coding?.itemsInPeriod?.length > 0) {
          console.log("📝 Coding Team items:", data.teams.coding.itemsInPeriod.slice(0, 3));
        }
        if (data.teams?.writing?.itemsInPeriod?.length > 0) {
          console.log("📝 Writing Team items:", data.teams.writing.itemsInPeriod.slice(0, 3));
        }
...
// Update the TeamCard component to show ALL items
const TeamCard = ({ meta }) => {
  ...
  return (
    <div className="glass-card rounded-2xl border border-gray-800 overflow-hidden mb-6">
      <div className="p-6 cursor-pointer hover:bg-gray-900/30 transition-all" onClick={() => toggleTeamExpansion(meta.id)}>
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <div className={`p-3 rounded-xl ${meta.color} bg-opacity-20 mr-4`}>
          ...
          <div className="flex items-center space-x-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-white">{teamData.total}</div>
              <div className="text-sm text-gray-400">Total</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-cyan-400">{teamData.active}</div>
              <div className="text-sm text-gray-400">Active</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-400">{teamData.completed}</div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          <div className="p-4 bg-gray-900/50 rounded-xl">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold text-white">{teamData.total}</span>
            </div>
          </div>
          <div className="p-4 bg-gray-900/50 rounded-xl">
            <div className="flex items-center justify-between">
              <span className="text-gray-400">Completed</span>
              <span className="text-lg font-bold text-cyan-400">{teamData.completed}</span>
            </div>
          </div>
          <div className="p-4 bg-gray-900/50 rounded-xl">
            <div className="flex items-center justify-between">
              <span className="text-gray-400">Active</span>
              <span className="text-lg font-bold text-purple-400">{teamData.active}</span>
            </div>
          </div>
        </div>
      </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-800">
                        ...
                        <th className="text-left py-3 px-4 text-gray-400 font-medium">Taken By</th>
                    ...
                    <tbody>
                      {teamData.itemsInPeriod.map((item) => {
                        // Extract data
                        let title = item.title || item.projectName || item.proposalTitle || 
                                   item.paperTitle || item.journalTitle || item.name || 
                                   item.documentTitle || item.id || "Untitled";
                        
                        let status = item.status || item.state || "-";
                        let takenBy = item.takenBy || item.assignedTo || item.author || 
                                     item.createdBy || item.assignee || item.uploadedBy || 
                                     item.responsiblePerson || "Unassigned";
                        
                        const userImg = getAvatarForName(takenBy) || 
                                       `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(String(takenBy))}`;
                        
                        return (
                          <tr key={item.id} className="border-b border-gray-900 hover:bg-gray-900/30">
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 p-4 md:p-6">
                ...
                {timeFilterOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`font-medium ${timeFilter === option.id ? "text-white" : "text-gray-300"}`}>
                        {option.label}
                      </span>
                    </button>
                  );
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="p-3 bg-gray-900/30 rounded-lg">
                    ...
                    <div className="text-xl font-bold text-white">{reportData.overall?.totals?.total ?? 0}</div>
                  </div>
                  <div className="p-3 bg-gray-900/30 rounded-lg">
                    <div className="text-sm text-gray-400">Active</div>
                    <div className="text-xl font-bold text-cyan-400">{reportData.overall?.totals?.active ?? 0}</div>
                  </div>
                  <div className="p-3 bg-gray-900/30 rounded-lg">
                    <div className="text-sm text-gray-400">Completed</div>
                    <div className="text-xl font-bold text-green-400">{reportData.overall?.totals?.completed ?? 0}</div>
                  </div>
                </div>
              </div>
            )}
        ...
        {isLoading && (
          <div className="glass-card rounded-2xl p-6 mb-6 border border-gray-800 text-center">
            ...
            <div className="text-gray-400">Loading reports from Firebase...</div>
          </div>
        )}
